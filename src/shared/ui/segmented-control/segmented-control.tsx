...
export default function SegmentedControl({ controls }: ISegmentedControl) {
  ...
  return (
    <Section className="relative !flex-row gap-2 p-1 rounded-[10px] w-fit bg-zinc-100 dark:bg-zinc-950 dark:ring-1 dark:ring-zinc-900">
      {controls.map(({ name, icon }, index) => {
        ...
        return (
          ...
          >
            ...
            {isAcive && (
              <motion.div
                className="absolute bg-white dark:bg-zinc-900 top-0 left-0 right-0 w-full h-full rounded-md dark:border dark:border-zinc-800/50"
                layoutId="control"
